package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_1311:Dictionary;
      ...
      private const const_2388:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_1070 = new Timer(this.const_2388,1);
         this.var_1070.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_481)
         {
            ...
            this.var_481.addEventListener(AssetLoaderEvent.const_41,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_481)
         {
            ...
            this.var_481.removeEventListener(AssetLoaderEvent.const_41,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_1069 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1310);
            Logger.log("AVATAR ASSET ERROR: Could not download figuremap.xml.");
         }
         else
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_1069);
            if(this.var_1310.indexOf("?") > 0)
            {
               _loc2_ = this.var_1310 + "&retry=" + this.var_1069;
            }
            else
            {
               _loc2_ = this.var_1310 + "?retry=" + this.var_1069;
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_2040 = true;
         for each(_loc2_ in this.var_1312)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2737,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1311[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_2040)
         {
            this.var_1312.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_561[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_561[_loc3_] = _loc6_;
            }
            this.var_1071[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_1071)
         {
            _loc7_ = true;
            _loc4_ = this.var_1071[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_561[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_1071[_loc3_];
         }
         while(_loc6_ < this.var_861.length)
         {
            _loc5_ = this.var_861[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_861.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_1070.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_860.length > 0 && this.var_861.length < this.const_1788)
         {
            _loc1_ = this.var_860[0];
            _loc1_.startDownloading();
            this.var_861.push(this.var_860.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_860.indexOf(param1) == -1 && this.var_861.indexOf(param1) == -1)
         {
