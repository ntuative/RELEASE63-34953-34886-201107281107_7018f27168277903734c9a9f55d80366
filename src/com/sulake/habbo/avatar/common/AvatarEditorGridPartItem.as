package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_1618:IWindow;
      
      private var var_874:IFigurePartSet;
      ...
      private var var_715:Rectangle;
      
      private var var_2069:int = 0;
      
      private var var_482:IAvatarRenderManager;
      
      private var var_1154:Boolean;
      
      private var var_2071:BitmapData;
      
      private const const_2398:int = 13421772;
      
      private const const_2399:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_874 = param3;
         this._window = param1;
         this.var_1618 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_2069 = Math.max(this.var_2069,_loc6_.colorLayerIndex);
            }
         }
         this.var_482 = this.var_78.controller.manager.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_760 != null && !this.var_2070)
            {
               _loc3_ = this.var_760;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_874)
            {
               switch(this.var_874.clubLevel)
               {
                  case HabboClubLevelEnum.const_54:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_34:
                     _loc2_.style = HabboIconType.const_995;
                     break;
                  case HabboClubLevelEnum.const_36:
                     _loc2_.style = HabboIconType.const_904;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1618.color = this.const_2398;
         }
         else
         {
            this.var_1618.color = this.const_2399;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_482.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_482.isFigureReady(_loc1_))
         {
            this.var_482.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_389[_loc6_] + "_" + FigureData.const_153;
               _loc4_ = this.var_482.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_715 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_2071)
               {
                  ...
                  this.var_2071 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_715.width,this.var_715.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_715.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_715.y;
               if(this.var_2070 && _loc2_.colorLayerIndex > 0)
               {
