package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridColorItem
   {
      ...
      private var var_1114:IPartColor;
      ...
      private function updateThumbData() : void
      {
         ...
         if(_loc1_ != null)
         {
            _loc1_.bitmap = new BitmapData(this.var_625.width,this.var_625.height,true,0);
            _loc1_.bitmap.copyPixels(this.var_625,this.var_625.rect,new Point(0,0));
         }
         var _loc2_:IBitmapWrapperWindow = this._window.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
         if(_loc2_ != null)
         {
            _loc2_.bitmap = new BitmapData(this.var_957.width,this.var_957.height,true,0);
            _loc5_ = this.var_957.clone();
            _loc5_.colorTransform(_loc5_.rect,this.var_1114.colorTransform);
         ...
         if(_loc3_ != null)
         {
            if(this._isSelected)
            {
               _loc3_.bitmap = new BitmapData(this.var_956.width,this.var_956.height,true,16777215);
               _loc3_.bitmap.copyPixels(this.var_956,this.var_956.rect,new Point(0,0),null,null,true);
               _loc3_.visible = true;
            }
            else
            {
         ...
         if(_loc4_)
         {
            if(this.var_1114)
            {
               switch(this.var_1114.clubLevel)
               {
                  case HabboClubLevelEnum.const_54:
                     _loc4_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_34:
                     _loc4_.style = HabboIconType.const_995;
                     _loc4_.visible = true;
                     break;
                  case HabboClubLevelEnum.const_36:
                     _loc4_.style = HabboIconType.const_904;
                     _loc4_.visible = true;
               }
            }
            else
