package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class SpacesNewCatalogWidget extends ItemGridCatalogWidget implements IDisposable, ICatalogWidget, IItemGrid
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.switchCategory(this.var_119[this.var_572]);
      ...
      {
         var _loc2_:int = this.var_692[this.var_572];
         var _loc3_:Offer = this.var_224[this.var_572][_loc2_];
      ...
      {
         var _loc2_:* = null;
         if(param1 > -1 && param1 < var_512.numGridItems)
         {
            _loc2_ = this.var_224[this.var_572][param1];
      ...
      override public function select(param1:IGridItem) : void
      {
         ...
         this.var_692[this.var_572] = (this.var_224[this.var_572] as Array).indexOf(_loc2_);
         this.updateRoomPreview();
      }
      
      private function updateRoomPreview() : void
      {
         var _loc1_:int = this.var_692[0];
         var _loc2_:int = this.var_692[1];
         var _loc3_:int = this.var_692[2];
         var _loc4_:Offer = this.var_224[0][_loc1_];
         var _loc5_:Offer = this.var_224[1][_loc2_];
         var _loc6_:Offer = this.var_224[2][_loc3_];
      ...
      private function createOfferGroups() : Boolean
      {
         ...
         {
            if(!(_loc1_.pricingModel == Offer.const_423 || _loc1_.pricingModel == Offer.const_452))
            {
               continue;
            }
            ...
            while(this.var_224.length < this._groupNames.length)
            {
               this.var_224.push(new Array());
            }
            switch(_loc4_)
            {
               case "floor":
                  (this.var_224[_loc5_] as Array).push(_loc1_);
                  break;
               case "wallpaper":
                  (this.var_224[_loc5_] as Array).push(_loc1_);
                  break;
      ...
      private function switchCategory(param1:String) : void
      {
         ...
         if(_loc2_ > -1)
         {
            ...
            if(var_512)
            {
               var_512.destroyGridItems();
            }
            page.replaceOffers(this.var_224[this.var_572],false);
            resetTimer();
            populateItemGrid();
            ...
            _loc3_ = this.var_692[this.var_572];
