package com.sulake.habbo.ui.widget.furniture.dimmer
{
   ...
   public class DimmerViewColorGrid
   {
      ...
      public function setSelectedColorIndex(param1:int) : void
      {
         ...
         if(param1 < 0 || param1 >= this.var_601.numGridItems)
         {
            return;
         }
         this.select(this.var_601.getGridItemAt(param1) as IWindowContainer);
      }
      
      private function populate(param1:IHabboWindowManager) : void
      {
         if(this._view == null || this.var_601 == null)
         {
            return;
         }
      ...
      private function populateColourGrid(param1:IHabboWindowManager) : void
      {
         ...
         var _loc13_:* = null;
         var _loc14_:* = null;
         this.var_601.destroyGridItems();
         this.var_239 = null;
         for each(_loc2_ in this.colors)
         {
            _loc3_ = param1.buildFromXML(this.var_2183) as IWindowContainer;
            ...
            _loc3_.width = this.var_743.width;
            _loc3_.height = this.var_743.height;
            this.var_601.addGridItem(_loc3_);
            _loc4_ = _loc3_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(this.var_743.width,this.var_743.height,true,0);
               _loc4_.bitmap.copyPixels(this.var_743,this.var_743.rect,new Point(0,0));
            }
            _loc5_ = _loc3_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(this.var_1393.width,this.var_1393.height,true,0);
            ...
            if(_loc6_ != null)
            {
               _loc6_.bitmap = new BitmapData(this.var_1127.width,this.var_1127.height,true,16777215);
               _loc6_.bitmap.copyPixels(this.var_1127,this.var_1127.rect,new Point(0,0),null,null,true);
      ...
      {
         var _loc2_:int = this.var_601.getGridItemIndex(param1.target as IWindow);
