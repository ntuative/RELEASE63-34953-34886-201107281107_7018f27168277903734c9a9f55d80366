package com.sulake.habbo.ui.widget.avatarinfo
{
   ...
   public class OwnAvatarMenuView extends AvatarInfoView
   {
      ...
      private static const const_1055:int = 8;
      ...
      public static function setup(param1:OwnAvatarMenuView, param2:int, param3:String, param4:int, param5:int, param6:AvatarInfoData) : void
      {
         ...
         {
            param1.var_701 = const_683;
         }
         else if(param6.allowNameChange && param1._widget.useMinimizedOwnAvatarMenu)
         {
            param1.var_701 = const_1056;
         }
         else
         {
            param1.var_701 = const_471;
         }
      ...
      private function showButton(param1:String, param2:Boolean = true, param3:Boolean = true) : void
      {
         ...
         _loc6_.textColor = !!param3 ? uint(const_2074) : uint(const_2071);
         var _loc7_:IIconWindow = _loc5_.getChildByName("icon") as IIconWindow;
         if(_loc7_)
         {
            _loc7_.color = !!param3 ? uint(const_2072) : uint(const_2073);
            if(_loc7_.tags.indexOf("arrow_left") != -1)
            {
               _loc7_.x = _loc6_.x + (_loc6_.width - _loc6_.textWidth) / 2 - _loc7_.width - const_1055;
            }
            if(_loc7_.tags.indexOf("arrow_right") != -1)
            {
               _loc7_.x = _loc6_.x + (_loc6_.width + _loc6_.textWidth) / 2 + const_1055;
            }
         }
      }
      ...
      private function buttonEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK)
         {
            if(param2.name == "button")
            {
               ...
               {
                  case "change_name":
                     _loc4_ = new RoomWidgetUserActionMessage(RoomWidgetUserActionMessage.const_498);
                     break;
                  case "change_looks":
                  ...
                  case "dance_4":
                     ...
                     break;
                  case "effects":
                     _loc4_ = new RoomWidgetRequestWidgetMessage(RoomWidgetRequestWidgetMessage.const_414);
                     HabboTracking.getInstance().trackEventLog("OwnAvatarMenu","click","effects");
                     break;
                  case "back":
                     _loc3_ = false;
                     this.changeMode(const_471);
                     break;
                  case "more":
                     _loc3_ = false;
         ...
         else if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_OVER)
         {
            if(param2.name == "button")
            {
               param2.color = const_1040;
            }
            else if(param2.tags.indexOf("link") > -1)
            {
               if(param2.tags.indexOf("actions") > -1)
               {
                  ITextWindow(IWindowContainer(param2).getChildAt(0)).textColor = const_1432;
               }
            }
         }
