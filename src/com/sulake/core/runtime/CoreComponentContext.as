package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1573:String = "asset-libraries";
      
      private static const const_1572:String = "library";
      
      private static const const_1574:String = "service-libraries";
      
      private static const const_1571:String = "library";
      
      private static const const_1576:String = "component-libraries";
      
      private static const const_1577:String = "library";
      
      private static const const_1575:String = "error_data";
      
      private static const const_1579:String = "error_cat";
      
      private static const const_1578:String = "error_desc";
       
      
      private var var_397:LibraryLoaderQueue;
      
      private var var_851:IEventDispatcher;
      
      private var var_571:uint;
      
      private var var_398:Function;
      
      private var var_223:Profiler;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         {
            this.var_293.push(new Array());
            this.var_698.push(0);
            _loc3_++;
         }
         this._lastUpdateTimeMs = getTimer();
         attachComponent(this,[new IIDCore()]);
         var_436.addEventListener(Event.ENTER_FRAME,this.onEnterFrame);
         switch(param2 & 0)
         {
            case Core.const_1351:
               debug("Core; using simple frame update handler");
               this.var_398 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_818:
               debug("Core; using complex frame update handler");
               this.var_398 = this.complexFrameUpdateHandler;
               break;
            case Core.const_860:
               debug("Core; using profiler frame update handler");
               this.var_398 = this.profilerFrameUpdateHandler;
               this.var_223 = new Profiler(this);
               attachComponent(this.var_223,[new IIDProfiler()]);
               var_436.addChild(new ProfilerViewer(this.var_223));
               break;
            case Core.const_637:
               debug("Core; using experimental frame update handler");
               this.var_398 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_398 = this.debugFrameUpdateHandler;
         }
      }
      ...
      {
         return this.var_571 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            try
            {
               ...
               {
                  receivers = this.var_293[i] as Array;
            ...
            if(var_436)
            {
               var_436.removeEventListener(Event.ENTER_FRAME,this.onEnterFrame);
               var_436 = null;
            }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1575,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1579,String(param3));
         ErrorReportStorage.setParameter(const_1578,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_851 = loadingEventDelegate;
         if(this.var_397 == null)
         {
            this.var_397 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1573)[0];
            if(node != null)
            {
               list = node.child(const_1572);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_234,e);
         }
         try
         {
            node = config.child(const_1574)[0];
            if(node != null)
            {
               list = node.child(const_1571);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_234,e);
         }
         try
         {
            node = config.child(const_1576)[0];
            if(node != null)
            {
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_234,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1234);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_851)
         {
            this.var_851.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_851)
         {
            ...
            this.var_851.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_851)
                  {
                     this.var_851.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_571 - this.var_397.length,this.var_571,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_223 ? 0 : this.var_1575 & 0;
         if(_loc3_ == Core.const_637)
         {
            this.var_293[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_293[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc4_:int = !!this.var_223 ? 0 : this.var_1575 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_108)
         {
            _loc3_ = this.var_293[_loc5_] as Array;
            if(_loc4_ == Core.const_637)
            {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_398(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         {
            this.var_698[priority] = 0;
            receivers = this.var_293[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         var maxTimePerFrame:uint = 1000 / var_436.stage.frameRate;
         priority = 0;
         while(priority < CoreComponentContext.const_108)
         {
            t = getTimer();
            ...
            {
               if(this.var_698[priority] < priority)
               {
                  ++this.var_698[priority];
                  skip = true;
               }
            }
            if(!skip)
         ...
         while(priority < CoreComponentContext.const_108)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     this.var_223.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
                     error("Error in update receiver \"" + getQualifiedClassName(receiver) + "\": " + e.message,true,e.errorID,e);
      ...
      public function setProfilerMode(param1:Boolean) : void
      {
         ...
         else
         {
            if(this.var_223)
            {
               detachComponent(this.var_223);
               this.var_223.dispose();
               this.var_223 = null;
            }
            switch(this.var_1575 & 0)
            {
               case Core.const_1351:
                  this.var_398 = this.simpleFrameUpdateHandler;
                  break;
