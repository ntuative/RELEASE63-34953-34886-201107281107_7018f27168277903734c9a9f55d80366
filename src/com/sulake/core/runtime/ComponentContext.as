package com.sulake.core.runtime
{
   ...
   public class ComponentContext extends Component implements IContext
   {
      ...
      protected var var_317:Array;
      
      protected var _debug:Boolean = false;
      
      protected var var_436:DisplayObjectContainer;
      ...
      public final function loadFromFile(param1:URLRequest, param2:LoaderContext) : LibraryLoader
      {
         ...
         var _loc3_:int = 0;
         while(_loc3_ < this.var_317.length)
         {
            if(this.var_317[_loc3_].url == param1.url)
            {
               return this.var_317[_loc3_].loadFromFile(param1);
            }
            _loc3_++;
         }
      ...
      protected function removeLibraryLoader(param1:LibraryLoader) : void
      {
         ...
         {
            if(this.var_317[_loc2_] == param1)
            {
               this.var_317.splice(_loc2_,1);
               break;
            }
            _loc2_++;
         }
         ...
         while(i < componentList.length())
         {
            ...
            if(assetCount > 0)
            {
               ...
               while(k < assetCount)
               {
                  ...
                  {
                     this.error("Manifest for component " + componentString + " contains broken asset \"" + assetName + "\"!",true,Core.const_861);
                  }
                  k++;
               }
            }
            ...
            if(componentClass == null)
            {
               this.error("Invalid component class " + componentString + "!",true,Core.const_861);
               return null;
            }
            ...
            if(component != null)
            {
               if(assetLibrary != null)
               {
                  if(component.assets != assetLibrary)
                  {
                     ...
                     this.error("Component \"" + componentString + "\" did not save provided asset library!",true,Core.const_861);
                  }
               }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            if(this.var_317 != null)
            {
               _loc1_ = uint(this.var_317.length);
               while(_loc1_-- > 0)
               {
                  _loc2_ = this.var_317[0];
                  this.removeLibraryLoader(_loc2_);
                  _loc2_.dispose();
               }
               this.var_317 = null;
